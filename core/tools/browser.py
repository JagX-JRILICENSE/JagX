...
def browser_find_links(max_results: int = 40) -> str:
    ...
    for i in range(count):
        link=links.nth(i); label=(link.inner_text(timeout=3000) or "").strip().replace("\n"," "); href=link.get_attribute("href") or ""
        if label or href: rows.append(f"{i}: {label[:100]} -> {href[:300]}")
    return "\n".join(rows) or "No links found."

def _is_sensitive(target) -> bool:
    input_type=(target.get_attribute("type") or "").lower(); name=((target.get_attribute("name") or "")+" "+(target.get_attribute("aria-label") or "")+" "+(target.get_attribute("placeholder") or "")).lower()
...
def browser_click(selector: str) -> str:
    ...
    if _is_sensitive(target): return "Blocked: credential/verification controls cannot be automated by normal browser_click."
...
def browser_type(selector: str, text: str, clear: bool = True) -> str:
    page=_ensure(); target=page.locator(selector).first
    if _is_sensitive(target): return "Blocked: use browser_fill_saved_credential for a stored password; OTP and verification codes remain manual."
    if clear: target.fill(text)
    else: target.type(text)
    return f"Entered text into: {selector}"

def browser_fill_saved_credential(account: str, selector: str = "input[type='password']") -> str:
    if get_credential is None: return "Secure credential integration unavailable."
    page=_ensure(); result=get_credential(account)
    if not result.startswith("SECURE_CREDENTIAL:"): return "Saved credential unavailable for that account."
    secret=result[len("SECURE_CREDENTIAL:"):]
    try:
        target=page.locator(selector).first; input_type=(target.get_attribute("type") or "").lower()
        if input_type!="password": return "Blocked: the selected field is not a password field."
        target.fill(secret); return "Stored credential filled into the selected password field. The password was not returned or logged."
    finally: secret=""; result=""
...
def browser_download(selector: str, save_path: str = "") -> str:
    page=_ensure()
    with page.expect_download(timeout=30000) as info: page.locator(selector).first.click(timeout=10000)
    download=info.value
    if save_path:
...
def browser_close() -> str:
    ...
    try:
        ...
        if _pw: _pw.stop()
    finally: _pw=_browser=_context=_page=None
