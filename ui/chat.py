...
class JagXChat:
    ...
    def _button(self, parent, text, command, width=None):
        return tk.Button(parent, text=text, command=command, anchor="w", relief="flat", bd=0,
    ...
    def _build(self):
        ...
        self.status = tk.Label(top, text="● Ready", bg=PANEL, fg=GOOD, font=("Segoe UI", 10, "bold"))
        ...
        tk.Label(nav, text="COMMAND CENTER", bg=PANEL, fg=MUTED, font=("Segoe UI", 9, "bold")).pack(anchor="w", padx=14, pady=(15, 8))
        for label, prompt in [
            ("⌂  Assistant", "What can you help me do on Windows?"),
            ("⚡  Action Center", "Show me useful actions I can perform right now."),
            ("📁  Files & Apps", "Show me how to manage my files and applications."),
            ("🌐  Browser", "Help me control the browser."),
            ("💻  Coding", "Help me code, test, debug, or build a project."),
            ("🖼  Image Studio", "Create an image for me."),
            ("🧠  Memory", "Show me and manage what you remember about me."),
            ("📸  Screenshots", "Take a screenshot and understand what is on my screen."),
        ]:
        ...
        self._button(nav, "🎙  Voice assistant", self.listen).pack(fill="x", padx=8, pady=5)
        tk.Label(nav, text="Sensitive and destructive actions require confirmation.", wraplength=185, justify="left", bg=PANEL, fg=MUTED, font=("Segoe UI", 8)).pack(padx=14, pady=14)
        ...
        self._section(left, "RUNNING APPS", "Refreshes with the dashboard")
        self.apps = scrolledtext.ScrolledText(left, height=8, wrap=tk.WORD, bg=PANEL, fg=TEXT, relief="flat", font=("Consolas", 9))
        self.apps.pack(fill="x", pady=(0, 8))
        self._section(left, "RECENT / HOME FILES", "Quick access")
        self.files = scrolledtext.ScrolledText(left, height=8, wrap=tk.WORD, bg=PANEL, fg=TEXT, relief="flat", font=("Consolas", 9))
        self.files.pack(fill="both", expand=True)

        right = tk.Frame(lower, bg=BG, width=390)
        right.pack(side="left", fill="y", padx=(5, 0))
        right.pack_propagate(False)
        self._section(right, "ACTION CENTER", "Ask JagX to perform a task")
        actions = [
            ("Open File Explorer", "Open my file explorer."),
            ("Open Browser", "Open my web browser."),
            ("Show Screenshot", "Take a screenshot of my screen."),
            ("Check Ollama", "Check my local Ollama connection and model."),
            ("Project Status", "Inspect the current project and show its Git status."),
            ("Remember Something", "Help me save a preference to memory."),
        ]
        for label, prompt in actions:
            self._button(right, "⚡ " + label, lambda p=prompt: self.quick(p)).pack(fill="x", pady=3)
        self._section(right, "COMMAND / CHAT", "Natural language works everywhere")
        self.chat = scrolledtext.ScrolledText(right, wrap=tk.WORD, bg=PANEL, fg=TEXT, relief="flat", font=("Segoe UI", 9), height=14)
        self.chat.pack(fill="both", expand=True)

        composer = tk.Frame(content, bg=PANEL)
        composer.pack(fill="x", pady=(10, 0))
        self.entry = tk.Entry(composer, bg=CARD, fg=TEXT, insertbackground=TEXT, relief="flat", font=("Segoe UI", 11))
        self.entry.pack(side="left", fill="x", expand=True, padx=8, pady=8, ipady=8)
        ...
        tk.Button(composer, text="Send", command=self.send, bg=ACCENT, fg="white", relief="flat", width=8, pady=8, font=("Segoe UI", 10, "bold")).pack(side="left", padx=8)
    ...
    def _first_run(self):
        ...
        if not marker.exists():
            ...
            self._append("JagX", "Welcome back. Your Windows command center is ready.")
    ...
    def _listen_worker(self):
        try:
            ...
            text = ""
        if not text:
            self.root.after(0, lambda: self._append("JagX", "I didn't catch that. Please type your request."))
    ...
    def _refresh_dashboard(self):
        def worker():
            ...
            try:
                import psutil
                cpu = f"{psutil.cpu_percent(interval=0.4):.0f}%"
                ram = f"{psutil.virtual_memory().percent:.0f}%"
                processes = sorted(psutil.process_iter(["name", "memory_percent"]), key=lambda p: p.info.get("memory_percent") or 0, reverse=True)[:10]
                app_text = "\n".join(f"{p.info.get('name','?')}  •  {p.info.get('memory_percent',0):.1f}% RAM" for p in processes)
            except Exception as exc:
                app_text = f"Unable to read process list: {exc}"
            home = Path.home()
            try:
                items = list(home.iterdir())[:30]
                file_text = "\n".join(("📁 " if p.is_dir() else "📄 ") + p.name for p in items)
            except Exception as exc:
                file_text = f"Unable to read home folder: {exc}"
            self.root.after(0, lambda: self.cpu_card.configure(text=cpu))
            self.root.after(0, lambda: self.ram_card.configure(text=ram))
            self.root.after(0, lambda: self.apps_replace(app_text))
            self.root.after(0, lambda: self.files_replace(file_text))
            self.root.after(0, self._refresh_ai_status)
        threading.Thread(target=worker, daemon=True).start()

    def apps_replace(self, text):
        self.apps.delete("1.0", tk.END); self.apps.insert(tk.END, text)

    def files_replace(self, text):
        self.files.delete("1.0", tk.END); self.files.insert(tk.END, text)

    def _refresh_ai_status(self):
        try:
            llm = getattr(self.agent, "llm", None)
            model = getattr(llm, "model", "qwen2.5")
            self.model_card.configure(text=str(model))
            self.ai_status.configure(text=f"● AI: {model}", fg=GOOD)
        except Exception:
            self.ai_status.configure(text="● AI: local model status unavailable", fg=WARN)
